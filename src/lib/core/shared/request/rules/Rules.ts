...
import type { ValidationRuleResult, ValidationParams, ValidationMessage, ValidationErrorsMap } from '../BaseRequest.js';
...
export class Rules<M extends BaseValidationMessages> {
	constructor(private readonly validationMessages: M) {}

	private checkVal<Val>(val: Val) {
	...
	boolean<T = unknown, D = unknown>(params?: ValidationRuleParams<'boolean', { expected?: boolean }>): ValidationRuleResult<T, D> {
	...
	phone<T = unknown, D = unknown>(params?: ValidationRuleParams<'phone', { country: CountryCode }>): ValidationRuleResult<T, D> {
	...
	lettersOnly<T = unknown, D = unknown>(params?: ValidationRuleParams<'lettersOnly', { whiteSpaces?: boolean }>): ValidationRuleResult<T, D> {
	...
	allowedOnly<T = unknown, D = unknown>(params?: ValidationRuleParams<'allowedOnly', { allowed?: unknown[] }>): ValidationRuleResult<T, D> {
	...
	sameAs<T = unknown, D = unknown>(params: ValidationRuleParams<'sameAs', { key: keyof T | string }>): ValidationRuleResult<T, D> {
	...
	notSameAs<T = unknown, D = unknown>(params: ValidationRuleParams<'sameAs', { key: keyof T | string }>): ValidationRuleResult<T, D> {
	...
	required<T = unknown, D = unknown, J = unknown>(
		...
		return ({ val, listValues, key }: ValidationParams<T, D, J>): ValidationMessage | undefined => {
			if (!this.checkVal(val)) return message();
	...
	password<T = unknown, D = unknown>(
		...
		} = { ...params, message: { ...this.validationMessages.password, ...params?.message } };
		return ({ val }: ValidationParams<T, D>): ValidationMessage | undefined => {
			if (!this.checkVal(val)) return undefined;
